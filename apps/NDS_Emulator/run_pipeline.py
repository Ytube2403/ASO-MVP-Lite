...
def calculate_relevancy(row, config):
    ...
    # Custom Nintendo Ecosystem Focus Boost
    nintendo_terms = [
        "nds", "ds", "nintendo", "mario", "pokemon", "zelda", "gba", "gameboy", 
        "game boy", "nes", "snes", "n64", "super nintendo", "wii", "gamecube", 
        "metroid", "kirby", "donkey kong", "kart"
    ]
    if any(term in kw for term in nintendo_terms):
        score += 0.15
...
df_quality_log = pd.DataFrame(getattr(shortlist_result, "quality_log", []))
df_diversity_log = pd.DataFrame(getattr(shortlist_result, "diversity_log", []))
...
if confirmed_selection:
    ...
    selected_core_feature = selected_feature[:5]
...
metrics = [
    ...
    ("  of which Diversity Cap Events (CLUSTER_CAP_REACHED)", len(df_diversity_log)),
    ("Selector Quality Warnings", len(df_quality_log)),
]
...
sheets_info = [
    ...
    ("13_Top_By_Volume", "Top 30 clean keywords sorted by Volume (excluding intent mismatches, typos, and competitor brands)"),
    ("14_Not_Selected_Audit", "Every candidate not selected for the main shortlist, with a specific rejection reason (includes CLUSTER_CAP_REACHED diversity events)"),
    ("15_Selector_Quality_Log", "Selector-level warnings, e.g. SAFE_POOL_EXHAUSTED when target_count could not be filled with safe keywords")
]
...
for r in range(4, 4 + len(metrics)):
...
# --- 13_Top_By_Volume ---
ws_tpv = wb.create_sheet(title="13_Top_By_Volume")
df_clean = df[df['Bucket'] != 'Dropped']
df_tpv = df_clean.sort_values(by=['Volume', 'BalancedScore', 'KEI', 'Difficulty'], ascending=[False, False, False, True]).head(30)
for col_idx, col in enumerate(cols_curated, 1):
    ws_tpv.cell(row=1, column=col_idx, value=col)
for row_idx, (_, row) in enumerate(df_tpv.iterrows(), 2):
    for col_idx, col in enumerate(cols_curated, 1):
