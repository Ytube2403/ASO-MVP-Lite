...
All app runners must use the shared v4.5 `keyword_filter` engine. Truncation handling is centralized there so complete tokens such as `emoji`, `icon`, `sound`, `filter`, and `widget` are not dropped as broken prefixes across apps or locales.
